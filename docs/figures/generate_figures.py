...
def fig1_tsr_concept():
    ...
    # Draw table
    table_verts = [[(-0.5, -0.3, 0), (0.5, -0.3, 0), (0.5, 0.3, 0), (-0.5, 0.3, 0)]]
    ...
    draw_frame(ax1, np.array([0, 0, 0.1]), np.eye(3), scale=0.15, labels=["", "", ""])
    ...
    # Draw table
    ax2.add_collection3d(Poly3DCollection(table_verts, alpha=0.3, facecolor="brown", edgecolor="black"))
    ...
    ax2.plot_surface(x_disk, y_disk, z_disk, alpha=0.4, color="#32CD32", edgecolor="none")
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "fig1_tsr_concept.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
...
def fig2_coordinate_frames():
    ...
    draw_frame(ax, world_origin, np.eye(3), scale=0.4, labels=["$x_w$", "$y_w$", "$z_w$"])
    ...
    draw_frame(ax, tsr_origin, R_tsr, scale=0.3, labels=["$x_{tsr}$", "$y_{tsr}$", "$z_{tsr}$"])
    ...
    draw_frame(ax, ee_origin, R_ee, scale=0.2, labels=["$x_e$", "$y_e$", "$z_e$"])
    ax.text(
        ee_origin[0],
        ee_origin[1],
        ee_origin[2] + 0.25,
    ...
    box_collection = Poly3DCollection(faces, alpha=0.15, facecolor="green", edgecolor="green", linewidth=1)
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "fig2_coordinate_frames.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
...
def fig3_bounds_matrix():
    ...
    for i, (ax, title, color) in enumerate(zip(axes[1], titles_rot, colors)):
        ...
        ax.plot(0.7 * np.cos(arc_theta), 0.7 * np.sin(arc_theta), color=color, linewidth=3)
        ax.annotate(
            "",
            xy=(0.7 * np.cos(np.pi / 4), 0.7 * np.sin(np.pi / 4)),
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "fig3_bounds_matrix.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
...
def fig4_primitives():
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "fig4_primitives.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
...
def fig5_tsr_chain():
    ...
    for angle, alpha in [(0, 0.3), (-np.pi / 4, 0.5), (-np.pi / 2, 0.8)]:
        ...
        door_face = Poly3DCollection([rotated_corners], alpha=alpha * 0.3, facecolor="brown", edgecolor="black")
...
orientation:
  ...
  yaw: free       # any rotation
    ...
    # Draw table
    table_verts = [[(-0.4, -0.4, 0), (0.4, -0.4, 0), (0.4, 0.4, 0), (-0.4, 0.4, 0)]]
    table = Poly3DCollection(table_verts, alpha=0.4, facecolor="#8B4513", edgecolor="black", linewidth=2)
    ...
    ax2.quiver(0, -0.25, 0.25, 0, 0, -0.15, color="red", arrow_length_ratio=0.3, linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig(
        OUTPUT_DIR / "fig6_placement_example.png",
        dpi=150,
        bbox_inches="tight",
        facecolor="white",
        edgecolor="none",
