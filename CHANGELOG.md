...
- **Torus minor-angle, reach, clearance, and coverage semantics** (#71). Torus-side
  ...
  with `finger_too_short` when `finger_length < tube_radius + clearance`. Torus-span
  ...
  (default `[−π/2, +π/2]`) makes the represented set explicit; it must lie within the
  externally accessible **outer half** — inner-hole approaches are documented as out
  of scope and raise `ValueError`. Torus-side straddle feasibility uses the
