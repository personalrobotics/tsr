...
CYL_OFF = (-0.30, 0.0, 0.0)  # cylinder on the far left
SPH_OFF = (-0.10, 0.0, SPH_R)  # sphere (center raised to sit on floor)
TOR_OFF = (0.10, 0.0, TOR_r)  # torus  (center raised to sit on floor)
BOX_OFF = (0.30, 0.0, 0.0)  # box on the far right
...
def main() -> None:
    ...
    # Cylinder grasps
    cyl_pose = np.eye(4)
    cyl_pose[:3, 3] = CYL_OFF
    cyl_templates = gripper.grasp_cylinder(cylinder_radius=MUG_R, cylinder_height=MUG_H, reference="mug")
    cyl_poses, cyl_colors = _collect(cyl_templates, cyl_pose, max(1, round(TARGET / len(cyl_templates))))

    # Sphere grasps
    sph_pose = np.eye(4)
    sph_pose[:3, 3] = SPH_OFF
    sph_templates = gripper.grasp_sphere(object_radius=SPH_R, reference="ball")
    sph_poses, sph_colors = _collect(sph_templates, sph_pose, max(1, round(TARGET / len(sph_templates))))

    # Torus grasps
    tor_pose = np.eye(4)
    tor_pose[:3, 3] = TOR_OFF
    tor_templates = gripper.grasp_torus(torus_radius=TOR_R, tube_radius=TOR_r, reference="handle")
    tor_poses, tor_colors = _collect(tor_templates, tor_pose, max(1, round(TARGET / len(tor_templates))))

    # Box grasps
    box_pose = np.eye(4)
    box_pose[:3, 3] = BOX_OFF
    box_templates = gripper.grasp_box(box_x=BOX_X, box_y=BOX_Y, box_z=BOX_Z, reference="box")
    box_poses, box_colors = _collect(box_templates, box_pose, max(1, round(TARGET / len(box_templates))))
    ...
    TSRVisualizer(
        title=(
            f"Task Space Regions — cylinder ({n_cyl}) · sphere ({n_sph}) · "
